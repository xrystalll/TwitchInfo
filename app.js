(document => {
    ...
    const template = {
                    ...
                    <div class="user-profile-content">
                                ...
                                <div class="info">  
                                    <div class="uname animated fadeInUpBig">
                                        <a href="https://www.twitch.tv/${name}" target="_blank">
                                            <h4 title="${name} - id ${id}">${display_name} ${type === 'staff' ? template.ic_staff('#9147ff', 20) : ''}</h4>
                                        </a>
                                    </div>
                        ...
                        <div class="include">
                            <div class="container container-profile user-posts__container">
                                <div class="more_info">
                                    <span>Last seen: </span>
                                    <b>${timeFormat(new Date(lastSeen).getTime())}</b>
                                </div>
                                <div class="more_info">
                                    <span>Registration: </span>
                                    <b>${timeFormat(new Date(registration).getTime())}</b>
                                </div>
                                <div class="follows">FOLLOWED CHANNELS</div>
                                <div id="follows" class="more_info"></div>
                            </div>
                        </div>
                    </div>
                </article>
            ...
            return `
                <a href="?login=${name}" class="follow_item">
                    ...
                    <div class="follow_item_right">
                        ...
                        <div class="follow_date">Followed from ${timeFormat(new Date(followDate).getTime())}
                            ${notifications ? template.ic_notif_on('#ADADB8', 14) : template.ic_notif_off('#ADADB8', 14)}
                        </div>
        ...
        ic_staff: (color = '#fff', size = 24) => {
            return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24"><path clip-rule="evenodd" d="M0 0h24v24H0z" fill="none"/><path d="M22.7 19l-9.1-9.1c.9-2.3.4-5-1.5-6.9-2-2-5-2.4-7.4-1.3L9 6 6 9 1.6 4.7C.4 7.1.9 10.1 2.9 12.1c1.9 1.9 4.6 2.4 6.9 1.5l9.1 9.1c.4.4 1 .4 1.4 0l2.3-2.3c.5-.4.5-1.1.1-1.4z" fill="${color}"/></path></svg>`;
        }
    };
            ...
            data => {
                data.users.length !== 0 ? (
                    output.innerHTML = template.profile(
                        ...
                        data.users[0].type,
                        data.users[0].bio,
                        data.users[0].created_at,
                        data.users[0].updated_at,
                        data.users[0].logo
                    ),
